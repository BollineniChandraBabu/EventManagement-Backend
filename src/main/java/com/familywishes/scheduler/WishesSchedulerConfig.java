...
public class WishesSchedulerConfig {
    ...
    JobDetail wishesJobDetail() {
        return JobBuilder.newJob(WishesScheduler.class).withIdentity("dailyWishJob").storeDurably().build();
    }
    ...
    Trigger wishesTrigger(JobDetail wishesJobDetail) {
        return TriggerBuilder.newTrigger().forJob(wishesJobDetail).withIdentity("dailyWishTrigger")
                .withSchedule(CronScheduleBuilder.dailyAtHourAndMinute(6, 10)).build();
    }
