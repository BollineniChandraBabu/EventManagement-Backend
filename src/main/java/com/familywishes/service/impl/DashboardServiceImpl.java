...
public class DashboardServiceImpl implements DashboardService {
  ...
  public DashboardResponse getDashboard() {
    ...
    long emailsSentToday =
        emailLogRepository.countByStatusAndSentAtGreaterThanEqualAndSentAtLessThan(
            EmailStatus.SENT, startOfDay, startOfNextDay);
  ...
  public List<Map<String, Object>> getChart(int days) {
    LocalDateTime start = LocalDate.now(ZoneId.of("Asia/Kolkata")).minusDays(days).atStartOfDay();

    return igMessageLogRepository.getDailyCounts(start).stream()
        .map(
            r ->
                Map.of(
                    "date", r[0],
